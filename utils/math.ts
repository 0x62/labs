import { ProjectileState, SolvedVariables, TrajectoryPoint, Step } from '../types';
...
export const solveProjectileSystem = (inputs: ProjectileState): SolvedVariables | null => {
  ...
  // t = (2 * u * sin(theta)) / g  => u = (g * t) / (2 * sin(theta))
  ...
  // CASE E: Range and Flight Time provided (Theta and U unknown)
  // ux = Range / t
  // uy (initial) for ground-to-ground: t = 2*uy / g => uy = g*t / 2
...
export const generateSteps = (
  ...
  // 2. Solving for Angle (theta) - Special case if unknown
  ...
  // 8. Time to Max Height
    ...
    const tPeak = solved.flightTime / 2; // Assuming level ground for simplicity in explanation logic, or use vy=0
    steps.push({
