...
export const generateTrajectory = (vars: SolvedVariables, points = 100): TrajectoryPoint[] => {
  ...
  for (let i = 0; i <= points; i++) {
    ...
    const vy = vars.uy - G * t;
    data.push({ x, y: y < 0 ? 0 : y, t, vy });
  }
...
export const generateSteps = (
  ...
  if (target === "u") {
    if (inputs.range !== null && inputs.theta !== null) {
      steps.push({
        explanation:
          "We know the Range ($R$) and Launch Angle ($\\theta$). We can use the range equation.",
      ...
      steps.push({
        explanation:
          "We know the Maximum Height ($H$) and Angle ($\\theta$). We can use the max height equation.",
      ...
      steps.push({
        explanation:
          "We know Flight Time ($t$) and Angle ($\\theta$). Use the time of flight equation.",
      ...
      steps.push({
        explanation:
          "We know Range ($R$) and Flight Time ($t$), but not the angle or velocity. We must find components first.",
  ...
  // 2. Solving for Angle (theta)
    ...
    if (inputs.range !== null && inputs.flightTime !== null) {
      steps.push({
        explanation:
          "We calculated the horizontal and vertical velocity components ($u_x, u_y$) from Range and Time.",
  ...
  // 5. Solving for Time of Flight (t)
    ...
    steps.push({
      explanation:
        "The time of flight is determined by the vertical component of velocity ($u_y$).",
